            ...
            if neo_result:
                ...
                for index, record in enumerate(neo_result):
                    ...
                    with st.expander(f"Transaction Details {index + 1}"):
                        ...
                        st.write(f"Fraud Status: {'Fraudulent' if t['isFraud'] == '1' else 'Not Fraudulent'}")

                net = Network(height="500px", width="100%", notebook=True)
                for record in neo_result:
                    t = record['t']
                    # Add nodes with sent and received amounts
                    net.add_node(t['nameOrig'], label=f"{t['nameOrig']} (Sent: ${t['amount']})")
                    net.add_node(t['nameDest'], label=f"{t['nameDest']} (Received: ${t['amount']})")
                    # Add edge
                    net.add_edge(t['nameOrig'], t['nameDest'])
